...
class MyDataLoader(Dataset):
    ...
    def load_data(self,K,sigma=0):
        ...
        RIS_feature_1 = np.zeros((H.shape[0],self.M,2*self.N,self.K))
        RIS_feature_1[:,:,:self.N,:] = H.real
        RIS_feature_1[:,:,self.N:,:] = H.imag
        RIS_feature_1 = RIS_feature_1.transpose((0,2,1,3))
        RIS_feature_1 = torch.Tensor(RIS_feature_1)
        RIS_feature_1 = F.normalize(RIS_feature_1,dim=3)
        ...
        user_feature_1 = torch.Tensor(user_feature_1)
        user_feature_1 = F.normalize(user_feature_1,dim=1)
        

        RIS_feature_2 = np.zeros((C.shape[0],self.L,2*self.N,self.K))
        RIS_feature_2[:,:,:self.N,:] = C.real
        RIS_feature_2[:,:,self.N:,:] = C.imag
        RIS_feature_2 = RIS_feature_2.transpose((0,2,1,3))
        RIS_feature_2 = torch.Tensor(RIS_feature_2)
        RIS_feature_2 = F.normalize(RIS_feature_2,dim=3)
        ...
        user_feature_2 = torch.Tensor(user_feature_2)
        user_feature_2 = F.normalize(user_feature_2,dim=1)
